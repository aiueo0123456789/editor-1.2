...
export class StateModel_BaseEditForBezierModifier {
    constructor() {
        this.名前 = "ベジェモディファイア_ベース編集";
    ...
    async update(stateData) {
        ...
        if (activeView.mouseState.click && this.options[0]) {
            stateData.activeObject.addBaseVertices([activeView.mouseState.positionForGPU, vec2.addR(activeView.mouseState.positionForGPU,[100,0]), vec2.addR(activeView.mouseState.positionForGPU,[-100,0])]);
        }
        if (activeView.mouseState.click && this.options[1]) {
            stateData.activeObject.subBaseVertices(stateData.activeObject.selectIndexs);
        }
