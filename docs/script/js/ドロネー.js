...
function cdt(boundaryPoints, holeBoundaryPoints, option) {
	...
	while(!option.softcn) {
		...
		// 辺と閉境界との交差判定
		var resultCrossTri = cdt.getCrossTricn(points, head, cst);
...
cdt.generateInputData = function (inputBoundaryPoints, inputHoleBoundaryPoints, option) {
	...
	// 2. cnの作成
	var cn = new Array(boundaryPoints.length + holeBoundaryPoints.length);
	var cntBnd = 0;
	// 閉境界は cst[0]=cst[length-1] となるようにする
	for(var i = 0; i < boundaryPoints.length; ++i) {
		cn[i] = new Array(boundaryPoints[i].length);
	...
	for(var i = 0; i < holeBoundaryPoints.length; ++i) {
		cn[i + offset] = new Array(holeBoundaryPoints[i].length);
		for(var j = 0; j < cn[i + offset].length; ++j) {
			cn[i + offset][j] = cntBnd;
			++cntBnd
		}
		cn[i + offset].push(cn[i + offset][0]);
	}
	...
	if(merge) {
		...
		if(duplicateMap.length > 0) {
			...
			for(var ci = 0; ci < cn.length; ++ci) {
				for(var cj = 0; cj < cn[ci].length; ++cj) {
					cn[ci][cj] = indexMap[cn[ci][cj]];
				}
			}
		}
...
cdt.getCrossTricn = function (points, head, cst) {
...
// 引数 cn: 閉境界を構成する点のＩＤ[pointID1, pointID2,.... ] 
cdt.DelaunayTriangle.prototype.addPoint = function (newPointID, points, cn) {
	...
	while(stack.length != 0) {
		...
		cdt.DelaunayTriangle.swapping(stack, newPointID, points, cn);
		++count_swap;
	}
...
cdt.DelaunayTriangle.swapping = function (stack, newPointID, points, cn) {
