...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    for (var c = 0u; c < 3u; c ++) {
        ...
        for (var animationIndex = 0u; animationIndex < allocationArray[objectIndex].MAX_ANIMATIONS; animationIndex ++) {
            if (!isNaN(animations[(animationBufferStartIndex + animationIndex * allocationArray[objectIndex].MAX_VERTICES) * 3 + c].x) && !isNaN(animations[(animationBufferStartIndex + animationIndex * allocationArray[objectIndex].MAX_VERTICES) * 3 + c].y)) {
                add += animations[(animationBufferStartIndex + animationIndex * allocationArray[objectIndex].MAX_VERTICES) * 3 + c] * weights[allocationArray[objectIndex].weightBufferOffset + animationIndex];
            }
        }
        let fixVertexIndex = allocationArray[objectIndex].vertexBufferOffset + vertexIndex;
        rendering[fixVertexIndex * 3 + c] = base[fixVertexIndex * 3 + c] + add;
    }
}