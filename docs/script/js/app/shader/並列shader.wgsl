...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (allocationArray[objectIndex].parentType == 2) { // 親がベジェモディファイア
        ...
        let bezierIndex = weightGroup.indexs[0] + bezierAllocationArray[allocationArray[objectIndex].parentIndex].vertexBufferOffset; // ベジェのindex
    ...
    } else if (allocationArray[objectIndex].parentType == 3) { // 親がボーンモディファイア
        let weightGroup = weightGroups[fixVertexIndex];
