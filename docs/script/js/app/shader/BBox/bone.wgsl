...
@group(0) @binding(1) var<storage, read_write> aaa: array<atomic<i32>, 4>;
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>, @builtin(local_invocation_id) local_id: vec3<u32>) {
    ...
    if (index < vertexCount) {
        ...
        for (var i = 0u; i < threadProcessNum; i ++) {
            let fixIndex = index + i;
            if (fixIndex < vertexCount) {
                let flagsArrayIndex = fixIndex / 32u;
                let flagBitIndex = fixIndex % 32u;
                if (getBoolFromBit(flagsArrayIndex, flagBitIndex)) {
                    let position = vertices[fixIndex].h;
