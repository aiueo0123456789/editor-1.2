...
function vecReverseScaleR(vec1, value) {
    return [vec1[0] / value, vec1[1] / value];
}
...
self.onmessage = function(event) {
    ...
    const pixelDensity = e.pixelDensity;
    const collectedLines = [];

    const vecFix = (vec) => {
        return [vec[0], imageBufferSize[1] - vec[1]];
    }
    ...
    for (let i = e.startIndex; i < e.endIndex; i ++) {
        ...
        for (const line of lines) {
            collectedLines.push([
                vecReverseScaleR(vecFix(vecAdd([x, y], line[0])), pixelDensity),
                vecReverseScaleR(vecFix(vecAdd([x, y], line[1])), pixelDensity),
            ]);
        }
        index ++;
    }
