...
class WebGPU {
    ...
    getStructByteSize(struct) {
        let sumByte = 0;
        for (const data of struct) {
            if (data == "u32" || data == "f32") {
                sumByte += 4;
            } else if (data == "bit") {
                sumByte += 1;
            }
    ...
    createBitData(array, struct) {
        ...
        const buffer = new ArrayBuffer(bufferLength * this.getStructByteSize(struct));
    ...
    rangeTo4(offset, num) {
        ...
        let endByte = offset + num;
        startByte = Math.floor(startByte / 4) * 4;
        endByte = Math.ceil(endByte / 4) * 4;
        return [startByte, endByte];
    }

    deleteStructDataFromGPUBuffer(buffer, offset, deleteNum, struct) {
        const sourceOffset = offset * this.getStructByteSize(struct);
        const sizeToDelete = deleteNum * this.getStructByteSize(struct);
        const newBuffer = GPU.createBuffer((buffer.size - sizeToDelete), buffer.usage);
        GPU.copyBuffer(buffer, newBuffer, 0, 0, sourceOffset);
        GPU.copyBuffer(buffer, newBuffer, sourceOffset + sizeToDelete, sourceOffset, buffer.size - sourceOffset - sizeToDelete);
        return newBuffer;
    }
    ...
    copyBuffer(resource, copyTarget, resourceOffset = 0, targetOffset = 0, copySize = resource.size) {
        ...
        if (resourceOffset % 4 == 0 && targetOffset % 4 == 0 && copySize % 4 == 0) {
    ...
    async getStructDataFromGPUBuffer(sourceBuffer,struct,startIndex,readNum) {
        ...
        const structSize = this.getStructByteSize(struct); // 各フィールドのサイズが 4 バイト固定 (u32, f32)
        const result = [];
        let offset = 0;
    ...
    async getVerticesDataFromGPUBuffer(sourceBuffer,startIndex,readNum) {
        ...
        // 1. 読み出し用のバッファを作成
        const readbackBuffer = this.createBuffer(
            sizeToRead,
            GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
        );
    ...
    async consoleBufferData(buffer, struct, text = "", indexs = "all") {
        ...
        if (indexs == "all") {
            ...
            for (const index of indexs) {
                offset = index * (this.getStructByteSize(struct)); // フィールドのサイズを加算
                const keep = [];
                for (const field of struct) {
                    if (field === "u32") {
                        keep.push(dataView.getUint32(offset, true));
...
const copyForBytePipeline = GPU.createComputePipeline(
    ...
    fn main(@builtin(global_invocation_id) id: vec3<u32>) {
        let dstBitStartIndex = config.dstOffset;
        let dstBitEndIndex = dstBitStartIndex + config.copySize;

        let srcBitStartIndex = config.srcOffset;
        let srcBitEndIndex = srcBitStartIndex + config.copySize;

        let dstArrayStartIndex = dstBitStartIndex / 32u;
        let dstArrayEndIndex = ceilU32(dstBitEndIndex, 32u);
        if (dstArrayEndIndex - dstArrayStartIndex < id.x) {
            return ;
        }
        let dstArrayIndex = dstArrayStartIndex + id.x;
        for (var i = 0u; i < 32u; i ++) {
            let dstBitIndex = dstArrayIndex * 32u + i;
            if (dstBitStartIndex <= dstBitIndex && dstBitIndex < dstBitEndIndex) {
                let srcBitIndex = dstBitIndex - dstBitStartIndex + srcBitStartIndex;
                let srcArrayIndex = srcBitIndex / 32u;
                let srcArrayInBitOffset = srcBitIndex % 32u;
                let bitValue = (src[srcArrayIndex] >> srcArrayInBitOffset) & 1u;
                dst[dstArrayIndex] = (dst[dstArrayIndex] & ~(1u << i)) | (bitValue << i);
            }
        }
    }
    `
